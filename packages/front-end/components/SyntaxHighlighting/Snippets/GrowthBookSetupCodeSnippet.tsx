...
export default function GrowPulseSetupCodeSnippet({
  language,
  version,
  apiKey,
  apiHost,
  ...
  if (language === "javascript") {
    return (
      <>
        ...
        />
        Create a GrowPulse instance. Read more about our{" "}
  ...
  if (language === "react") {
    ...
    return (
      <>
        ...
        />
        Wrap app in a GrowPulseProvider
        <Code
          language="tsx"
          code={`
import { useEffect } from "react";
import { GrowPulseProvider } from "@growpulse/growpulse-react";
...
export default function MyApp() {
  ...
  return (
    <GrowPulseProvider growthbook={growthbook}>
      <MyComponent/>
    </GrowPulseProvider>
  )
}
`.trim()}
        ...
        with examples of using GrowPulse with SSR, API routes, static pages,
        and more.
      </>
    );
  }
  if (language === "nodejs") {
    ...
    if (useMultiUser) {
      return (
        <>
          Create and initialize a GrowPulse client
          <Code
            language="javascript"
            code={`
const { GrowPulseClient } = require("@growpulse/growpulse");

const client = new GrowPulseClient({
...
await client.init({ timeout: 1000 });
          `.trim()}
          />
          Use a middleware to create a GrowPulse instance that is scoped to the
...
setPolyfills({
  ...
  EventSource: require("eventsource")
        ...
        `.trim()}
        />
        Add a middleware before any routes that will use GrowPulse
        <Code
          language="javascript"
          code={`
const { GrowPulse } = require("@growpulse/growpulse");

app.use(function(req, res, next) {
  // Create a GrowPulse Context
  req.growthbook = new GrowPulse({
    apiHost: ${JSON.stringify(apiHost)},
    clientKey: ${JSON.stringify(apiKey)},${
            encryptionKey
              ? `\n    decryptionKey: ${JSON.stringify(encryptionKey)},`
    ...
    return (
      <>
        ...
        <Code
          language="swift"
          code={`
var gb: GrowPulseSDK = GrowPulseBuilder(
  ...
  if (language === "go") {
    return (
      <>
        Create GrowPulse client instance
        <Code
          language="go"
          code={`
package main
...
func main() {
  ...
  if (language === "ruby") {
    return (
      <>
        Get features from the GrowPulse API
        <Code
          language="ruby"
          code={`
require 'growthbook'

# Fetch features from a GrowPulse instance
    ...
    print_r([
      ...
      "properties" => [
        ...
        "variationId" => $result->variationId
            ...
            `.trim()}
        />
        Load features from the GrowPulse API
        <Code
          language="php"
          code={`
...
def on_experiment_viewed(experiment, result):
  ...
  print("Variation Id: " + result.key)
            `.trim()}
        />
        Create a GrowPulse instance and load features
        <Code
          language="python"
          code={`
from growthbook import GrowPulse
...
GBContext context = GBContext.builder()
    ...
    .build();
GrowPulse growthBook = new GrowPulse(context);
            `.trim()
              : `
GBContext context = GBContext.builder()
    .featuresJson(featuresJson)
    ...
    return (
      <>
        ...
        <Code
          language="dart"
          code={`
final GrowPulseSDK gb = GBSDKBuilderApp(
  hostURL: '${apiHost}/',
  apiKey: "${apiKey}",
...
// Fetch feature flags from the GrowPulse API
...
var gb = new GrowPulse.GrowPulse(context);
    `.trim()}
        />
      </>
    );
...
features = GrowPulse.Config.features_from_config(features)
          `.trim()}
        />
      </>
    );
...
GROWTHBOOK_CLIENT_KEY=${JSON.stringify(apiKey)}${
        ...
        <p>
          Set up a <strong>Cloudflare KV</strong> store and use a GrowPulse{" "}
          <strong>SDK Webhook</strong> to keep feature and experiment values
          synced between GrowPulse and your Cloudflare Worker. This eliminates
          network requests from your edge to GrowPulse.
        </p>
...
async function handleRequest(event) {
  ...
  const config = {
    // Name of Fastly backend pointing to your GrowPulse API Endpoint
  ...
  return await gbHandleRequest(event.request, env, config);
        ...
        <p>
          ...
          GrowPulse API by setting up backends (origins) for your Compute
...
GROWTHBOOK_CLIENT_KEY=${JSON.stringify(apiKey)}${
        ...
        <p>
          Set up a <strong>Fastly KV</strong> store and use a GrowPulse{" "}
          <strong>SDK Webhook</strong> to keep feature and experiment values
          synced between GrowPulse and your Fastly worker. This eliminates
          network requests from your edge to GrowPulse.
        </p>
    ...
    GROWTHBOOK_CLIENT_KEY: ${JSON.stringify(apiKey)},${
        ...
        <ul>
          <li>
            ...
            use a GrowPulse <strong>SDK Webhook</strong> to keep feature and
            experiment values synced between GrowPulse and your edge worker.
            This eliminates network requests from your edge to GrowPulse.
          </li>
          <li>
            Enable URL Redirect experiments on edge (off by default) by setting{" "}
            <code>{`RUN_URL_REDIRECT_EXPERIMENTS="everywhere"`}</code>
...
GROWTHBOOK_CLIENT_KEY=${JSON.stringify(apiKey)}${
        ...
        <ul>
          <li>
            Set up an edge key-val store and use a GrowPulse{" "}
            <strong>SDK Webhook</strong> to keep feature and experiment values
            synced between GrowPulse and your edge worker. This eliminates
            network requests from your edge to GrowPulse.
          </li>
          <li>
            Enable URL Redirect experiments on edge (off by default) by setting{" "}
...
} from "@growpulse/growpulse/plugins";

const growthbook = new GrowPulse({
  apiHost: ${JSON.stringify(apiHost)},
  clientKey: ${JSON.stringify(apiKey)},${
      encryptionKey
        ? `\n  decryptionKey: ${JSON.stringify(encryptionKey)},`
  ...
  // Supports plugins, but with a different tracker
    ...
    jsCode = `
import { GrowPulse } from "@growpulse/growpulse";
import { autoAttributesPlugin } from "@growpulse/growpulse/plugins";
...
const growthbook = new GrowPulse({
  ...
  // No plugins support
    ...
    jsCode = `import { GrowPulse } from "@growpulse/growpulse";
