...
(function(s) {
  ...
  s.src="https://cdn.jsdelivr.net/npm/@growpulse/growpulse/dist/bundles/auto.min.js";
...
<script async
  ...
  src="https://cdn.jsdelivr.net/npm/@growpulse/growpulse/dist/bundles/auto.min.js"
></script>
            `.trim();
  ...
  const getInstallationCodeSnippet = useCallback(
    (language: SDKLanguage) => {
      if (eventTracker === "GTM" && clientSideLanguages.includes(language)) {
        return (
          <>
            Add the GrowPulse snippet to your Google Tag Manager as a Custom
            HTML tag.{" "}
      ...
      if (language === "nocode-shopify") {
        return (
          <>
            Add the GrowPulse snippet right before the closing{" "}
...
# OR
yarn add @growpulse/growpulse`.trim()}
          />
        );
      }
      if (language === "react") {
        return (
          <Code
            language="sh"
            code={`
npm i --save @growpulse/growpulse-react
# OR
yarn add @growpulse/growpulse-react`.trim()}
          />
        );
      }
      if (language === "nodejs") {
...
dependencies {
    implementation 'io.growthbook.sdk:GrowPulse:1.+'
}`.trim()}
          />
        );
      }
