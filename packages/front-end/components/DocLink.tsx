...
const docsOrigin = "https://docs.growpulse.io";
...
export function inferDocUrl() {
  ...
  if (page === "/") return "https://docs.growpulse.io";

  if (page.match(/^\/features/)) return docUrl("features");
  if (page.match(/^\/experiment\//)) return docUrl("experimentResults");
  if (page.match(/^\/experiments\//)) return docUrl("experimentConfiguration");
  if (page.match(/^\/metric\//)) return docUrl("metrics");
  if (page.match(/^\/metrics/)) return docUrl("metrics");
  if (page.match(/^\/fact-table/)) return docUrl("factTables");
  if (page.match(/^\/dimension/)) return docUrl("dimensions");
  if (page.match(/^\/segments/)) return docUrl("datasources");
  if (page.match(/^\/datasources/)) return docUrl("datasources");
  if (page.match(/^\/dashboard/)) return docUrl("experimentConfiguration");
  if (page.match(/^\/idea/)) return docUrl("experimentConfiguration");
  if (page.match(/^\/power-calculator/)) return docUrl("powerCalculator");
  if (page.match(/^\/sdks/)) return docUrl("api");
  if (page.match(/^\/event-webhooks/)) return docUrl("eventWebhooks");
  if (page.match(/^\/sdk-webhooks/)) return docUrl("sdkWebhooks");

  return "https://docs.growpulse.io";
}
...
export const docUrl = (docSection: DocSection, fallBackSection = "home") => {
  let path = docSections[docSection];
  if (!path && fallBackSection) {
    path = docSections[fallBackSection as DocSection];
  }
  // Using growpulse.io as documentation domain
  return "https://docs.growpulse.io" + path;
};
