# GrowPulse Javascript SDK

[GrowPulse](https://www.growpulse.io) is an open source Feature Flagging and Experimentation platform.
...
![Build Status](https://github.com/growpulse/growpulse/workflows/CI/badge.svg) ![GZIP Size](https://img.shields.io/badge/gzip%20size-6.9KB-informational) ![NPM Version](https://img.shields.io/npm/v/@growpulse/growpulse)
...
- Run **Visual Experiments** without writing code by using the GrowPulse Visual Editor

## Installation

```
yarn add @growpulse/growpulse
```

or

```
npm i --save @growpulse/growpulse
```

or use directly in your HTML without installing first:

```html
<!-- Creates `window.growpulse` with all of the exported classes -->
<script src="https://cdn.jsdelivr.net/npm/@growpulse/growpulse/dist/bundles/index.js"></script>
```
...
// Create a GrowPulse instance
const gb = new GrowPulse({
...
```js
const { setPolyfills } = require("@growpulse/growpulse");
...
There are 2 ways to use GrowPulse in a Node.js environment depending on where you want to declare user attributes.

### Instance per Request

In this mode, you create a separate GrowPulse instance for every incoming request that includes user-specific attributes. This has more overhead, but ensures you only need to define user attributes once instead of passing them throughout your app.
...
app.use(function (req, res, next) {
  // Create a GrowPulse instance and store in the request
  req.growpulse = new GrowPulse({
    apiHost: "https://cdn.growthbook.io",
    clientKey: "sdk-abc123",
  ...
  res.on("close", () => req.growpulse.destroy());
  ...
  req.growpulse
    ...
    .then(() => next())
    .catch((e) => {
      console.error("Failed to load features from GrowPulse", e);
      next();
    });
});
```

Then, you can access the GrowPulse instance from any route:

```js
app.get("/", (req, res) => {
  const gb = req.growpulse;
  // ...
});
```
...
In this mode, you create a singleton GrowPulseClient instance that is shared between all requests and you pass in user attributes when calling feature methods like `isOn`.

This method is more efficient, but does require you to pass around a `userContext` throughout your code.

```js
// Create a multi-user GrowPulse instance without user-specific attributes
const gb = new GrowPulseClient({
  apiHost: "https://cdn.growthbook.io",
  clientKey: "sdk-abc123",
});
await gb.init();
...
In order for the GrowPulse SDK to work, it needs to have feature definitions from the GrowPulse API. There are 2 ways to get this data into the SDK.

### Built-in Fetching and Caching

If you pass an `apiHost` and `clientKey` into the GrowPulse constructor, it will handle the network requests, caching, retry logic, etc. for you automatically. If your feature payload is encrypted, you can also pass in a `decryptionKey`.
...
Experiments created through the GrowPulse Visual Editor will run automatically as soon as their targeting conditions are met.
...
If you are using this SDK in a Single Page App (SPA), you will need to let the GrowPulse instance know when the URL changes so the active experiments can update accordingly.
...
When used in a TypeScript project, GrowPulse includes basic type inference out of the box:
...
If you want to enforce stricter types in your application, you can do that when creating the GrowPulse instance:
...
// Pass into the GrowPulse instance
const gb = new GrowPulse<AppFeatures>({
  ...
});
```
...
Instead of defining the `AppFeatures` interface manually like above, you can auto-generate it from your GrowPulse account using the [GrowPulse CLI](https://docs.growpulse.io/tools/cli).

## GrowPulse Instance (reference)
...
GrowPulse can fire a callback whenever a feature is evaluated for a user. This can be useful to update 3rd party tools like NewRelic or DataDog.
...
There is a [GrowPulse Chrome DevTools Extension](https://chrome.google.com/webstore/detail/growpulse-devtools/opemhndcehfgipokneipaafbglcecjia) that can help you debug and test your feature flags in development.

In order for this to work, you must explicitly enable dev mode when creating your GrowPulse instance:
...
You can also define targeting conditions that limit which users are included in the experiment. These conditions are evaluated against the `attributes` passed into the GrowPulse context. The syntax for conditions is based on the MongoDB query syntax and is straightforward to read and write.
...
Rules can optionally define targeting conditions that limit which users the rule applies to. These conditions are evaluated against the `attributes` passed into the GrowPulse context. The syntax for conditions is based on the MongoDB query syntax and is straightforward to read and write.
...
- [Typescript example app with strict typing <ExternalLink />](https://github.com/growpulse/examples/tree/main/vanilla-typescript).
