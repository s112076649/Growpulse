---
title: GrowPulse DevTools - Browser Extension for Feature Flags & A/B Testing
description: Learn how to use GrowPulse DevTools, a browser extension (Chrome and Firefox) for inspecting, debugging, and modifying feature flags and A/B tests in real time. Improve experiment workflows effortlessly.
...
GrowPulse DevTools is a browser extension for Chrome and Firefox that helps you inspect, debug, and modify feature flags and experiments in real time. Whether you're troubleshooting an experiment or tweaking a feature flag, DevTools gives you full visibility and control—right in your browser.
...
GrowPulse DevTools is compatible with Chrome (including Edge, Arc, and most other browsers that enable Chrome extensions) and Firefox.

It works with most GrowPulse client SDKs, including the HTML Script Tag, JavaScript SDK, and React SDK (client-rendered).
...
The GrowPulse DevTools browser extension is available for both Chrome and Firefox.

GrowPulse DevTools is available for Chrome on the [Chrome Web Store](https://chromewebstore.google.com/detail/growpulse-devtools/opemhndcehfgipokneipaafbglcecjia) and for Firefox on [addons.mozilla.org](https://addons.mozilla.org/en-US/firefox/addon/growpulse-devtools/). To install:

- Click **Add to Chrome** / **Add to Firefox**
- Confirm the installation when prompted

Once installed, GrowPulse DevTools automatically detects and connects to the GrowPulse SDK whenever it's active on a website ✨

## Accessing GrowPulse DevTools

GrowPulse DevTools is accessible in two ways. Use whichever best fits your workflow:

### 1. Quick Access

A lightweight view for quick flag and experiment testing.

- Pin the extension to your toolbar (click the puzzle piece 🧩, then pin GrowPulse).
- Click the GrowPulse icon to open the popup.

:::tip
A green dot on the GrowPulse icon means DevTools are successfully connected.
:::
...
- Click the **GrowPulse** tab in the Developer Tools panel. (If it's hidden, check the overflow menu ».)

![Chrome devtools panel](/images/devtools-panel.webp)

## Essential Features

GrowPulse DevTools provides powerful debugging and testing tools, including:
...
- View and edit the attributes the GrowthPulse SDK is using.
...
GrowPulse DevTools works great out of the box, but if you want even more power, enable advanced features with an Access Token:
...
- If self-hosting, uncheck **GrowPulse Cloud** and enter your API details.
- Click **Save**.
...
GrowPulse DevTools also powers our Visual Editor, which enables designing A/B tests without writing code. The Visual Editor requires the Access Token mentioned in the previous section. See the docs [on the Visual Editor.](https://docs.growthbook.io/app/visual)
